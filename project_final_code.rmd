...
This Chunk we add previous application status into our current data
```{r}
...
This Chunk we manually choose some columns 
```{r}
...
Missing Values:
* Remove columns where more than 40% data is na
* note: there is still na value in the dataset, here we just removed columns with too many na value.
*       We will remove the rest na value in a later cell.
```{r}
...
*  Capping means we change outlier with the upper limit or lower limit
```{r}
names(data1)
...
Missing value
*  This cell we remove all of observations contain NA value(removed only less than 1000 rows)
```{r}
...
*   2: create ordinal variables (Like 1,2,3 etc)
```{r}
library(mlr)
...
Since the two class(0 and 1) in our dataset are severely unbalance(10:1), so we'll use some
technique to balance these two class.
Since it might be computationally expensive to train the model if we do oversampling, here we use undersampling method.

```{r}
dim(data1)
#Here we remove the 4 correlated variables:
#  AMT_CREDIT, AMT_ANNUITY, CNT_FAM_MEMBERS, REGION_RATING_CLIENT_W_CITY
data1 <- subset(data1, select = -c(AMT_CREDIT, AMT_ANNUITY, CNT_FAM_MEMBERS, REGION_RATING_CLIENT_W_CITY
                                     ))
